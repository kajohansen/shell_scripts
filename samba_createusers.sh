	...
	do
		...
		if [ `echo "$first $last" | wc -w` -eq 2 ]; then
			...
			if [ `echo "$first" | wc -c` -gt 2 ] && [ `echo "$last" | wc -c` -gt 2 ]; then
				...
				if [ $verbose == "-v" ]; then
					echo "samba-tool user add \"$username\" \'pa\$\$w0rd!\'  --userou=\"ou=Standard Users,ou=People\" \
				...
				eval samba-tool user add \"$username\" \'pa\$\$w0rd!\'  --userou=\"ou=Standard Users,ou=People\" \
				     --surname=\"$surname\" --given-name=\"$name\" --initials=\"$initials\"  --company=\"EALL\" \
